...
| Road Conditions | Regional live | ARDOT, KDOT, MoDOT, ODOT WZDx, and TDOT SmartWay adapters use viewport filtering, provenance, validation, cache, stale fallback, and explicit outside-coverage state. ODOT can resolve its public feed through USDOT's registry without tester setup. |
| Public Cameras | Regional live | ARDOT, KDOT, MoDOT, Nebraska DOT 511, and TDOT SmartWay adapters are implemented. Wide views cluster cameras and zoom 6+ preserves individual points. Selecting a camera opens its media directly. Oklahoma cameras remain unavailable pending an official consumable feed. |
