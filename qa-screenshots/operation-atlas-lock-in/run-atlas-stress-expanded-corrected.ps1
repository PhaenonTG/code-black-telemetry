...
$timeline = Join-Path $OutDir 'atlas-memory-timeline-expanded-corrected.csv'
$statusPath = Join-Path $OutDir 'atlas-stress-status-expanded-corrected.json'
$logPath = Join-Path $OutDir 'atlas-stress-expanded-corrected-runner.log'
...
while (((Get-Date) - $start).TotalSeconds -lt 1800) {
  ...
  Tap 552 754; $followCycles++
  Tap 1090 626; Start-Sleep -Milliseconds 900; $expandedCycles++
...
adb -s $Serial logcat -d -v time AndroidRuntime:E chromium:W cr_AwContents:V Mapbox:D '*:S' > (Join-Path $OutDir 'atlas-logcat-filtered-expanded-corrected.txt')
Select-String -Path (Join-Path $OutDir 'atlas-logcat-filtered-expanded-corrected.txt') -Pattern 'tile memory|FATAL|ANR|SIGSEGV|OOM|WebGL context|EGL|Mapbox|chromium' | Set-Content (Join-Path $OutDir 'atlas-chromium-warnings-expanded-corrected.txt') -Encoding utf8
