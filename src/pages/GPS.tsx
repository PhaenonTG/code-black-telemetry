  ...
  return (
    <div className="cb-scroll flex-1 p-4 grid gap-4" style={{ gridTemplateColumns: "1fr 1fr 1fr", alignContent: "start" }}>
      ...
      <DashCard title="Heading" accent>
        <div className="flex flex-col items-center justify-center py-4">
          <span className="font-mono text-7xl font-bold text-cb-blue tabular-nums">
            {gps.headingDeg.toFixed(0)}
          </span>
      ...
      <DashCard title="Fix Status" accent>
        <div className="flex flex-col items-center justify-center py-4">
          <span className={`font-mono text-4xl font-bold ${gps.hasFix ? "text-cb-green" : "text-cb-red"}`}>
            {gps.hasFix ? "FIXED" : "NO FIX"}
          </span>
      ...
      <DashCard title="Position & Details" className="col-span-3">
        ...
        <MetricRow label="Speed"      value={gps.speedMph.toFixed(2)} unit="mph" />
        <MetricRow label="Heading"    value={`${gps.headingDeg.toFixed(1)}°`} />
